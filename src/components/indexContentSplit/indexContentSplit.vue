            ...
            <div class="leftBlock" ref="leftBlock">
              <Menu :theme="theme" width="100%" accordion @on-select="openTag">
                <Submenu name="5">
                  ...
                  <MenuItem name="51">数据看板</MenuItem>
                </Submenu>
                ...
                <Submenu name="4">
                  ...
                  <MenuItem name="43">生产排期查询</MenuItem>
                </Submenu>
              </Menu>
            </div>
          </div>
          ...
          <div class="right-split-pane">
            <div class="rightBlock" ref="rightBlock">
              <Tabs type="card" closable @on-tab-remove="handleTabRemove" :value="activeTab" name="indexContentSplit">
                <!-- 数据看板 -->
                <TabPane label="数据看板" v-if="isShowList['51']" name="51" tab="indexContentSplit">
                  <v-dataBoard></v-dataBoard>
                </TabPane>
                <!-- 用户设置 -->
                ...
                <TabPane label="走货一览表" v-if="isShowList['41']" name="41" tab="indexContentSplit">
                  <v-productionPlanningSetting></v-productionPlanningSetting>
                </TabPane>
                <!-- 排产品类 -->
                <TabPane label="生产排期查询" v-if="isShowList['43']" name="43" tab="indexContentSplit">
                  <v-productionScheduleEnquiry></v-productionScheduleEnquiry>
                </TabPane>
              </Tabs>
            </div>
          </div>
...
import productionPlanningSetting from "../productionPlanningSetting/productionPlanningSetting";
import colorSetting from "../colorSetting/colorSetting";
import argumentSetting from "../argumentSetting/argumentSetting";
import productionScheduleEnquiry from "../productionScheduleEnquiry/productionScheduleEnquiry";
...
export default {
  data: function() {
    return {
      ...
      nowShowingList: [51], // 用于重新加载 Tab 组件
      activeTab: "51"
    }
  },
  methods: {
  ...
  components: {
    ...
    'v-productionPlanningSetting': productionPlanningSetting,
    'v-colorSetting': colorSetting,
    'v-argumentSetting': argumentSetting,
    'v-productionScheduleEnquiry': productionScheduleEnquiry,
    'v-dataBoard': dataBoard,
  }
}
