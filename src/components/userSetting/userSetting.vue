...
/*          render: (h, params) => {
  ...
  created: function() {
    // 页面初始化设置高度
    this.$nextTick(() => {
      this.$refs["userSettingLeftBlock"].style.height = this.$refs["userSettingRightBlock"].style.height = this.windowHeight - CONST.cssData.pageHeaderHeight - CONST.cssData.containerPageTabHeight - CONST.cssData.rightBlockTabpaneWrapperPaddingBottom + "px";
      this.tableHeight = this.windowHeight - CONST.cssData.pageHeaderHeight - CONST.cssData.containerPageTabHeight - CONST.cssData.rightBlockTabpaneWrapperPaddingBottom - 50; // 50 是存放提交按钮区域的高度
    });
    this.reloadTree();
  }
}
