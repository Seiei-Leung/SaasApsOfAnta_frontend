        ...
        <div v-show="isShowSettingBlock">
          <div class="topBlock">
            ...
            <div class="inputBar">
              ...
              <div class="inputWrapper">
                <InputNumber :min="0" :step="0.01" v-model="inputProphaseLowEfficiency" style="width: 80px"></InputNumber>
              </div>
            </div>
          </div>
          <div class="bottomBlock">
            ...
            <div>
              ...
              <Modal v-model="isShowAddProductClass" v-bind:title="addProductClassTitle" @on-ok="addProductClass" @on-cancel="addProductClassCancel" ok-text="确认" cancel-text="取消">
                ...
                <div style="margin-top:20px;margin-left: 12px;">前三天效率：
                  <InputNumber :min="0" :step="0.01" v-model="inputProphaseLowEfficiency" style="margin-left: 31px;width: 100px"></InputNumber>
                </div>
              </Modal>
            </div>
          </div>
...
export default {
  data: function() {
    return {
      ...
      addProductClassTitle: "添加产品类",
      properityNameList: [],
      isShowAddProductClass: false, // 是否显示产品类输入框
      inputDefaultEffiency: 1, // 产品类默认效率
      inputProphaseLowEfficiency: 1, // 前三天效率
  ...
  methods: {
    ...
    addProductClass: function() {
      ...
      this.axios.get(this.seieiURL + "/productclass/insertProductClass", {
        params: {
          ...
          prophaseLowEfficiency: this.inputProphaseLowEfficiency
        }
      }).then((response) => {
        if (response.data.status) {
          that.$Message.error(response.data.msg);
    ...
    showModifyProductClass: function(data, index) {
      ...
      this.inputProphaseLowEfficiency = data.prophaseLowEfficiency;
    ...
    getOut: function() {
      ...
      this.inputDefaultEffiency = 1;
      this.inputProphaseLowEfficiency = 1;
    },
    // 显示删除产品类窗口
    ...
    addProductClassCancel: function() {
      this.isShowAddProductClass = false;
    }
  },
  created: function() {
    var that = this;
